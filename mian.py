...
if __name__ == '__main__':
    ...
    for epoch in range(epoch_start_idx, args.num_epochs + 1):
        ...
        for step, batch in tqdm(enumerate(train_loader), total=len(train_loader)):
            ...
            log_feats, pos_embs, neg_embs = model(seq, pos, neg, token_type, next_token_type, next_action_type, seq_feat, pos_feat, neg_feat)
            
            optimizer.zero_grad()
            
            loss = model.compute_infonce_weight_loss(log_feats, pos_embs, neg_embs, loss_mask, next_action_type)
        ...
        valid_loss_sum = 0
        with torch.no_grad():
            for step, batch in enumerate(tqdm(valid_loader, total=len(valid_loader))):
                seq, pos, neg, token_type, next_token_type, next_action_type, seq_feat, pos_feat, neg_feat = batch
                seq  = seq.to(args.device)
                pos  = pos.to(args.device)
                neg  = neg.to(args.device)
                next_action_type = next_action_type.to(args.device)
                loss_mask = (next_action_type == 1).to(args.device)

                log_feats, pos_embs, neg_embs = model(seq, pos, neg,
                                                    token_type, next_token_type, next_action_type,
                                                    seq_feat, pos_feat, neg_feat)
                loss = model.compute_infonce_weight_loss(log_feats, pos_embs, neg_embs,
                                                        loss_mask, next_action_type)
