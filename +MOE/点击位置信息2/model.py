...
class BaselineModel(torch.nn.Module):
    ...
    def __init__(self, user_num, item_num, feat_statistics, feat_types, args):  #
        ...
        user_dim = args.hidden_units * (len(self.USER_SPARSE_FEAT) + 2 + len(self.USER_ARRAY_FEAT)) + len(
    ...
    def feat2emb(self, seq, feature_array, mask=None, seq_action_types=None, include_user=False):
        """
        Args:
            seq: 序列ID
        ...
        if seq_action_types is not None:
            action_type_embedding = self.action_type_emb(seq_action_types.to(self.dev))
            user_feat_list.append(action_type_embedding)
    ...
    def log2feats(self, log_seqs, mask, seq_feature, seq_action_types=None):
        """
        Args:
            log_seqs: 序列ID
        ...
        seqs = self.feat2emb(log_seqs, seq_feature, mask=mask, seq_action_types=seq_action_types, include_user=True)
