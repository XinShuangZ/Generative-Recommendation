...
class BaselineModel(torch.nn.Module):
    ...
    def forward(
        ...
        neg_logits_for_infonce = torch.matmul(log_feats, neg_embedding_all.transpose(-1, -2))

        loss_mask = (next_mask == 1).to(self.dev)
        loss_mask_bool = loss_mask.bool()
        pos_logits = pos_logits_for_infonce[loss_mask_bool]
        neg_logits = neg_logits_for_infonce[loss_mask_bool]
        ...
        if writer is not None and self.training and global_step % 10 == 0:
            neg_dot_pos = (neg_embs * pos_embs).sum(dim=-1)
            # 正负向量相似度
            writer.add_scalar("'+' & '-'/mean(neg_embs * pos_embs)", neg_dot_pos.mean().item(),global_step)
            writer.add_scalar("'+' & '-'/std(neg_embs * pos_embs)", neg_dot_pos.std().item(),global_step)
            # 正分数
            writer.add_scalar("'pred' & '+' /mean(pred_embs * pos_embs)", pos_logits.mean().item(),global_step)
            writer.add_scalar("'pred' & '+' /std(pred_embs * pos_embs)", pos_logits.std().item(),global_step)
            # 负分数
            writer.add_scalar("'pred' & '-' /mean(pred_embs * neg_embs)", neg_logits.mean().item(),global_step)
            writer.add_scalar("'pred' & '-' /std(pred_embs * neg_embs)", neg_logits.std().item(),global_step)
        
        return pos_logits, neg_logits, final_action_types 
    ...
    def compute_triple_loss(self, pos_logits, neg_logits, writer=None, global_step=None):
        ...
        if writer is not None and self.training and global_step % 100 == 0:
            writer.add_scalar(f"'+' & '-'/neg_logits.mean() - pos_logits.mean()", delta_pctr.mean())
            writer.add_scalar(f"Loss(Train)/triple_loss:{self.args.Triple_Loss_lambda}", triple_loss.item(), global_step)
        
        return triple_loss
    
    def compute_infonce_loss_weighted(self, pos_logits, neg_logits, action_types, click_weight=6, expose_weight=1, writer=None, global_step=None):
        """
        计算加权的InfoNCE loss。
        点击行为产生的损失会被赋予更高的权重。
        Args:
            pos_logits (torch.Tensor): 正样本的logits，形状为 [M, 1]。
            neg_logits (torch.Tensor): 负样本的logits，形状为 [M, N_neg]。
            action_types (torch.Tensor): 每个正样本对应的行为类型，形状为 [M]。0表示曝光，1表示点击。
            click_weight (float): 点击行为的损失权重。
            expose_weight (float): 曝光行为的损失权重。
        Returns:
            torch.Tensor: 加权后的InfoNCE loss标量。
        """
        logits = torch.cat([pos_logits, neg_logits], dim=-1) / self.temperature
        labels = torch.zeros(logits.size(0), device=logits.device, dtype=torch.int64)
        per_sample_loss = F.cross_entropy(logits, labels, reduction='none')
        weights = torch.where(
            action_types == 1,  # 条件：行为类型是点击 (1)
            torch.tensor(click_weight, device=logits.device),  # 如果是点击，权重为 click_weight
            torch.tensor(expose_weight, device=logits.device)   # 如果是曝光，权重为 expose_weight
        )
        weighted_loss = per_sample_loss * weights
        final_loss = weighted_loss.mean()
        
        if self.training and writer is not None and global_step % 100 == 0:
            writer.add_scalar('Loss(Train)/infoNCE_loss_weighted', final_loss.item(), global_step)
    ...
    #                         fallback_neg_logits):
    #     """
    #     修改后的Triplet Loss，具有 fallback 机制。
    #     - 主要模式：优化排序，使得 点击得分 > 曝光得分。
    #     - Fallback模式：如果batch中没有点击事件，则退化为优化 正样本得分 > 全局负样本得分。

    #     Args:
    #         clicked_scores (torch.Tensor): 点击事件的得分，形状 [N_clicks]。
    #         exposed_scores (torch.Tensor): 曝光事件的得分，形状 [N_exposures]。
    #         fallback_pos_logits (torch.Tensor): 用于fallback的所有正样本logits，形状 [M, 1]。
    #         fallback_neg_logits (torch.Tensor): 用于fallback的所有全局负样本logits，形状 [M, N_neg]。

    #     Returns:
    #         torch.Tensor: 计算出的三元组损失标量。
    #     """
    #     # --- 主要模式：优化点击 vs 曝光排序 ---
    #     # 检查是否可以执行主要模式：必须同时存在点击和曝光样本
    #     if clicked_scores.numel() > 0 and exposed_scores.numel() > 0:
    #         # print("Executing Primary Triplet Loss Mode") # (用于调试)
    #         mean_exposed_score = exposed_scores.mean()
    #         delta = mean_exposed_score - clicked_scores
    #         loss_terms = torch.clamp(delta + self.args.margin, min=0)
    #         triple_loss = self.args.Triple_Loss_lambda * loss_terms.mean()
    #     # --- Fallback 模式：优化正样本 vs 全局负样本 ---
    #     else:
    #         if fallback_pos_logits.numel() == 0:
    #             return torch.tensor(0.0, device=self.dev)
    #         # 计算全局负样本的平均得分
    #         # fallback_neg_logits: [M, N_neg] -> neg_scores_mean: [M]
    #         neg_scores_mean = fallback_neg_logits.mean(dim=-1)
    #         pos_scores = fallback_pos_logits.squeeze(-1)
    #         delta = neg_scores_mean - pos_scores
    #         loss_terms = torch.clamp(delta + self.args.margin, min=0)
    #         triple_loss = self.args.Triple_Loss_lambda * loss_terms.mean()
    #     return triple_loss
