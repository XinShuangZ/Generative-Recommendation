...
if __name__ == '__main__':
    ...
    model = torch.compile(model, mode='reduce-overhead', fullgraph=False, dynamic=True)  #减少运行开销，加快计算
    ...
    for epoch in range(epoch_start_idx, args.num_epochs + 1):
        ...
        for step, batch in tqdm(enumerate(train_loader), total=len(train_loader)):
            ...
            if global_step % 100 == 1:
                ...
                param_sq_norms = torch.stack([torch.norm(p) ** 2 for p in params])
                total_param_norm = torch.sqrt(param_sq_norms.sum()).item()
                grads = [p.grad for p in params if p.grad is not None]
                if grads:
                    grad_sq_norms = torch.stack([torch.norm(g) ** 2 for g in grads])
                    total_grad_norm = torch.sqrt(grad_sq_norms.sum()).item()
                else:
                    total_grad_norm = 0.0
                total_grad_div_num_norm = (total_grad_norm / num_params) ** 0.5
                total_param_div_num_norm = (total_param_norm / num_params) ** 0.5
                writer.add_scalar('Gradients/grad_l2', total_grad_norm, global_step)
                writer.add_scalar(f'Gradients/grad_div_{num_params}_l2', total_grad_div_num_norm, global_step)
                writer.add_scalar('Parameters/param_l2', total_param_norm, global_step)
                writer.add_scalar(f'Parameters/param_div_{num_params}_l2', total_param_div_num_norm, global_step)
            
            if args.use_grad_clip:
                grad_norm = torch.nn.utils.clip_grad_norm_(model.parameters(), args.grad_clip)
                clip_count += 1
                writer.add_scalar('Gradients/clip_count', clip_count, global_step)
        ...
        writer.add_scalar('Valid/InfoNCE_loss', valid_loss_avg, global_step)        
