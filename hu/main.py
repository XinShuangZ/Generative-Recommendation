...
def get_args():
    ...
    #triple loss
    parser.add_argument('--Triple_Loss_lambda', default=0.1, type=float)
    ...
    for epoch in range(epoch_start_idx, args.num_epochs + 1):
        ...
        for step, batch in tqdm(enumerate(train_loader), total=len(train_loader)):
            ...
            pos_logits, neg_logits = model(
                seq, pos, neg, token_type, next_token_type, next_action_type, seq_feat, pos_feat, neg_feat, writer=writer, global_step=global_step
            )

            '''主体损失（InfoNCE损失）'''
            loss = model.compute_infonce_loss(pos_logits, neg_logits, writer=writer, global_step=global_step)
            
            '''三元损失'''
            if args.Triple_Loss_lambda != 0:
                loss += model.compute_triple_loss(pos_logits, neg_logits, writer=writer, global_step=global_step)
        ...
        valid_loss_sum = 0
        if args.use_MoE:
            valid_moe_loss_sum = 0
            if args.MoE_router_loss_weight != 0:
                valid_moe_loss_sum_with_decay = 0
        ...
        with torch.no_grad():
            for step, batch in tqdm(enumerate(valid_loader), total=len(valid_loader)):
                ...
                '''前向传播'''
                pos_logits, neg_logits, Load_balance_loss = model(
        ...
        writer.add_scalar('Valid/Loss(Valid)', valid_loss_sum, global_step)        
