...
class MyDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, uid):
        ...
        for record_tuple in user_sequence:
            ...
            if i and item_feat:
                item_sequence.append((i, item_feat, 1, action_type))
                
        max_item_len = self.maxlen if user_profile_record else self.maxlen + 1
        if len(item_sequence) > max_item_len:
            item_sequence = item_sequence[-max_item_len:]
        
        final_sequence = []
        if user_profile_record:
            final_sequence.append(user_profile_record)
        final_sequence.extend(item_sequence)
        ...
        ts = set()
        for record_tuple in final_sequence:
